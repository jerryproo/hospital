...
<script>
        layui.use(['table','laypage'], function() {
            ...
            table.render({
              elem: '#query'
              ,height: 500
              ,url: 'doct_showQuery?doct.doctId='+doctId //数据接口
              ,cols: [[ //表头
                {field: '', title: '序号', width:80, sort: true, fixed: 'left',templet:'#queryNo'}
                ,{ title: 'ID', width:80,templet:'#userId'}
                ,{field: '', title: '姓名', width:150, templet:'#userRealName'}
                ,{field: '', title: '性别', width:80 , templet:'#userSex'} 
                ,{field: '', title: '年龄', width: 80 , templet:'#userAge'}
                ,{field: '', title: '预约时间', width: 200, sort: true,templet:'#preDate'}
                ,{field: '', title: '签到时间', width: 200, sort: true,templet:'#comDate'}
                ,{field: '', title: '状态', width: 200,templet:'#udStatus'}
                ,{field: 'udNo', title: '排队序号', width: 200,sort: true}
              ]]
            });
       /*      //执行一个laypage实例
            laypage.render({
